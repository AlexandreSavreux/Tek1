...
static int my_optns_pick(char *optn, char *value, setting_t *settings)
{
	switch (optn[1]) {
	case 'w':
	...
	case 'p':
		return change_players_avatar(optn, value, settings);
	default:
...
static int optn_without_space(char *str, setting_t *settings)
{
	...
	return (str[1] == 'a') ? my_optns_pick(str, &str[2], settings) : 84;
}
