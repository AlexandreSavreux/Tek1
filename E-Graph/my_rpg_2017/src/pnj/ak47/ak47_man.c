...
static int check_attack_pnj(pnjs *pnj, sfSprite *chicken)
{
...
static void do_kitchen_move_ext(pnjs *pnj, sfVector2f *pos, sfVector2f scale)
{
	if (scale.x == -1.0 && pos->x >= 900 &&
		pnj[TERRORIST].hp[KITCHEN] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
		pos->x -= 1;
	else if (scale.x == -1.0 && pos->x <= 900 &&
		pnj[TERRORIST].hp[KITCHEN] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse) {
		pos->x -= 66;
...
static void do_kitchen_move_extend(pnjs *pnj, sfVector2f *pos, sfVector2f scale)
{
	if (scale.x == 1.0 && pos->x <= 1500 &&
		pnj[TERRORIST].hp[KITCHEN] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
		pos->x += 1;
	else if (scale.x == 1.0 && pos->x >= 1500 &&
		pnj[TERRORIST].hp[KITCHEN] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse) {
		scale.x = -1.0;
		pos->x += 66;
	...
	do_kitchen_move_ext(pnj, pos, scale);
}
...
void do_kitchen_move(pnjs *pnj, sfSprite *chicken)
{
	...
	if (pnj[TERRORIST].hp[KITCHEN] <= 0 && rect_t.top < 246)
		sfSprite_setTextureRect(pnj[TERRORIST].spr, rect);
	else if (check_attack_pnj(pnj, chicken) == 1) {
		manage_attack_pnj(pnj);
		ak47_pnj(pnj[TERRORIST].spr, pnj);
	} else
		pnj[TERRORIST].weapon[WEAPON].active = sfFalse;
	do_kitchen_move_extend(pnj, &pos, scale);
	if (pnj[TERRORIST].hp[KITCHEN] <= 0 ||
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
