...
static void manage_animation_pnj(pnjs *pnj)
{
	...
	if (pnj[TERRORIST].hp[WAREHOUSE] <= 0 &&
...
static int check_attack_pnj_warehouse(pnjs *pnj, sfSprite *chicken)
{
...
static void do_warehouse_move_ext(pnjs *pnj, sfVector2f scale, sfVector2f *pos)
{
	if (scale.x == -1.0 && pos->x >= 640 &&
		pnj[TERRORIST].hp[WAREHOUSE] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
		pos->x -= 1;
	else if (scale.x == -1.0 && pos->x <= 640 &&
		pnj[TERRORIST].hp[WAREHOUSE] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse) {
		pos->x -= 66;
...
static void do_warehouse_move_extend(pnjs *pnj, sfVector2f scale,
	sfVector2f *pos
)
{
	if (scale.x == 1.0 && pos->x <= 1280 &&
		pnj[TERRORIST].hp[WAREHOUSE] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
		pos->x += 1;
	else if (scale.x == 1.0 && pos->x >= 1280 &&
		pnj[TERRORIST].hp[WAREHOUSE] > 0 &&
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse) {
		scale.x = -1.0;
		pos->x += 66;
	...
	do_warehouse_move_ext(pnj, scale, pos);
}
...
void do_warehouse_move(pnjs *pnj, sfSprite *chicken)
{
	...
	if (pnj[TERRORIST].hp[WAREHOUSE] <= 0 && rect_t.top < 246)
		sfSprite_setTextureRect(pnj[TERRORIST].spr, rect);
	else if (check_attack_pnj_warehouse(pnj, chicken) == 1) {
		manage_attack_pnj_warehouse(pnj);
		ak47_pnj_warehouse(pnj[TERRORIST].spr, pnj);
	} else
		pnj[TERRORIST].weapon[WEAPON].active = sfFalse;
	do_warehouse_move_extend(pnj, scale, &pos);
	if (pnj[TERRORIST].hp[WAREHOUSE] <= 0 ||
		pnj[TERRORIST].weapon[WEAPON].active == sfFalse)
