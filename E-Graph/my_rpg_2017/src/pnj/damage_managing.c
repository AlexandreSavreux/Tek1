...
void damage_ak47_taliban(elem *elem, sfVector2f pos, sfVector2f pos_a)
{
	sfIntRect rect = sfSprite_getTextureRect(elem->chicken);

	if ((rect.top == 144 && pos.x > pos_a.x) ||
		(rect.top == 72 && pos.x < pos_a.x)) {
		if (check_time_ak47(&elem->pnj[TALIBAN].damage) == 0 &&
			elem->inv.user.item[AK47].weapon[WEAPON].active ==
				sfTrue)
			elem->pnj[TALIBAN].hp[elem->room] -= elem->stats.power;
	}
}

void damage_rocket_taliban(elem *elem, sfVector2f pos, sfVector2f pos_e)
{
	if ((pos.y >= pos_e.y && pos.y <= pos_e.y + 150) ||
		(pos.y + 123 >= pos_e.y && pos.y + 123 <= pos_e.y + 150)) {
		if (check_time_rocket(&elem->pnj[TALIBAN].damage) == 0 &&
			elem->inv.user.item[ROCKET].weapon[BULLET].active ==
				sfTrue)
			elem->pnj[TALIBAN].hp[elem->room] -= 100;
	}
}

void damage_knife_taliban(elem *elem, sfVector2f pos, sfVector2f pos_k)
{
	if ((pos_k.y >= pos.y && pos_k.y <= pos.y + 123) ||
		(pos_k.y + 10 >= pos.y && pos_k.y + 10 <= pos_k.y + 123)) {
		if (check_time_knife(&elem->pnj[TALIBAN].damage) == 0 &&
