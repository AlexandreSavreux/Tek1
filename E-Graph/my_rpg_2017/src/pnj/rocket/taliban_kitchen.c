...
static void do_taliban_kitchen_move_ext(pnjs *pnj, sfVector2f *pos,
	sfVector2f scale
)
{
	if (scale.x == -1.0 && pos->x >= 560 && pnj[TALIBAN].hp[KITCHEN] > 0 &&
...
static void do_taliban_kitchen_move_extend(elem *elem, sfVector2f *pos,
	sfVector2f scale
)
{
	if (elem->pnj[TALIBAN].hp[KITCHEN] <= 0 ||
		elem->pnj[TALIBAN].weapon[WEAPON].active == sfFalse) {
		manage_taliban_animation_pnj(elem->pnj);
	}
	if (scale.x == 1.0 && pos->x <= 1440 &&
		elem->pnj[TALIBAN].hp[KITCHEN] > 0 &&
		elem->pnj[TALIBAN].weapon[WEAPON].active == sfFalse)
		pos->x += 1;
	else if (scale.x == 1.0 && pos->x >= 1440 &&
		elem->pnj[TALIBAN].hp[KITCHEN] > 0 &&
		elem->pnj[TALIBAN].weapon[WEAPON].active == sfFalse) {
		scale.x = -1.0;
		pos->x += 66;
	...
	do_taliban_kitchen_move_ext(elem->pnj, pos, scale);
}
...
void do_taliban_kitchen_move(elem *elem, sfSprite *chicken)
{
	...
	if (elem->pnj[TALIBAN].hp[KITCHEN] <= 0 && rect_t.top < 245)
	...
	do_taliban_kitchen_move_extend(elem, &pos, scale);
	if (scale.x == -1)
		hp_pos.x -= 80;
